...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.7.21") {
        throw new Error(`Cannot start service: Host version "${"0.7.21"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
...
let startService = (options) => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.7.21"}`), {
    cwd: process.cwd(),
    windowsHide: true,
...
let runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.7.21"}`), {
    cwd: process.cwd(),
    windowsHide: true,
    input: stdin,
